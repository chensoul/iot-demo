...
public class DeviceService {
    ...
    @Transactional(rollbackFor = Exception.class)
    public Map<String, Object> report(HttpServletRequest request) throws IOException {
        return doReport(request, reqBody -> {
            //存储数据
        });
    }
    ...
    private Map<String, Object> doReport(HttpServletRequest request, Consumer<Map<String, Object>> consumer) throws IOException {
        Device device = (Device) request.getSession().getAttribute("device");
        if (device == null) {
            throw new IllegalArgumentException("设备不存在");
        }
        if (device.getStatus() == DeviceStatusEnum.UNACTIVE) {
            throw new IllegalArgumentException("设备未激活，不能上报");
        }

        Map<String, Object> reqBody = new ObjectMapper().readValue(request.getInputStream(), Map.class);
        if (reqBody != null && consumer != null) {
            consumer.accept(reqBody);
        }
        device.setLastOnlineTime(LocalDateTime.now());
        device.setStatus(DeviceStatusEnum.ONLINE);
        deviceRepository.save(device);
